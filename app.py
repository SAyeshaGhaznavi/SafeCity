...
def statistics_board():
    ...
    def report():
        ...
        if not is_anonymous:
            if not full_name: errors.append('Full name is required (or check "Report Anonymously").')
            if not cnic: errors.append('CNIC is required (or check "Report Anonymously").')
        ...
        db = get_db_connection()
        try:
            row = db.execute('SELECT category_id FROM CrimeCategories WHERE name = ?', (crime_type,)).fetchone()
            category_id = row['category_id'] if row else db.execute('INSERT INTO CrimeCategories (name) VALUES (?)',
                                                                    (crime_type,)).lastrowid

            user_id = session.get('user_id')
            if not is_anonymous:
                citizen = db.execute('SELECT citizen_id FROM Citizens WHERE cnic = ?', (cnic,)).fetchone()
                if citizen:
                    user_id = citizen['citizen_id']
                else:
                    cursor = db.execute('INSERT INTO Citizens (full_name, cnic) VALUES (?, ?)', (full_name, cnic))
                    user_id = cursor.lastrowid
                    session['user_id'] = user_id
            if is_anonymous:
                user_id = None
            ...
            cursor = db.execute(
                'INSERT INTO Complaints (user_id, category_id, description, location, status) VALUES (?, ?, ?, ?, ?)',
                (user_id, category_id, description, location, 'Pending'))
            complaint_id = cursor.lastrowid

            db.execute('INSERT INTO Notifications (message) VALUES (?)',
                       (f'New complaint #{complaint_id} ({crime_type}) reported at {location}.',))

            PRIORITY_MAP = {'Assault': 'High', 'Theft': 'Medium', 'Fraud': 'Medium', 'Cyber Crime': 'Medium',
                            'Vandalism': 'Low', 'Other': 'Medium'}
            priority = PRIORITY_MAP.get(crime_type, 'Medium')

            officer = db.execute(
                'SELECT ap.personnel_id FROM AuthorizedPersonnel ap LEFT JOIN Cases cs ON ap.personnel_id = cs.assigned_police_id AND cs.priority = \'High\' WHERE ap.type = \'Police\' GROUP BY ap.personnel_id ORDER BY COUNT(cs.case_id) ASC LIMIT 1').fetchone()
            assigned_police_id = officer['personnel_id'] if officer else None

            db.execute('INSERT INTO Cases (complaint_id, assigned_police_id, priority, notes) VALUES (?, ?, ?, ?)',
                       (complaint_id, assigned_police_id, priority, f'Auto-assigned | Priority: {priority}'))

            if evidence and evidence.filename:
                safe_name = secure_filename(evidence.filename)
                ext = safe_name.rsplit('.', 1)[-1] if '.' in safe_name else 'bin'
                stored_name = f"ev_{complaint_id}_{int(time.time())}.{ext}"
                evidence.save(os.path.join(app.config['UPLOAD_FOLDER'], stored_name))
                db.execute('INSERT INTO Evidence (complaint_id, file_url) VALUES (?, ?)',
                           (complaint_id, f"/uploads/{stored_name}"))
    ...
    def login():
        if request.method == 'POST':
            ...
            conn = get_db_connection()
            user = conn.execute("SELECT * FROM AuthorizedPersonnel WHERE email = ?", (email,)).fetchone()
            conn.close()
            ...
            if not user:
                ...
                error = True
            if user:
                if user['badge_number'] != badge_number: flash("Badge number is incorrect", "badge_error"); error = True
                if user['password_hash'] != password: flash("Password is incorrect", "password_error"); error = True
                if username and username != user['name']: flash("Username is incorrect", "username_error"); error = True

            if error: return render_template("login.html")

            # FIX: Set role in session to fix profile and dashboard routing
            session['user_id'] = user['personnel_id']
            session['name'] = user['name']
            session['role'] = user['type']  # 'Police', 'Detective', 'Operator'
    ...
    def register_badge():
        if request.method == 'POST':
            ...
            conn = get_db_connection()
            existing = conn.execute("SELECT * FROM AuthorizedPersonnel WHERE name = ? OR email = ?",
                                    (name, email)).fetchone()
            if existing:
                conn.close()
                return render_template("register_badge.html", msg="Name or Email already exists ❌")
            ...
            conn.execute(
                ...
                (name, email, password, badgenumber, 'Police'))
            conn.commit()
            conn.close()
            flash("Registered Successfully ✅", "success")
            return redirect(url_for("splash"))
    ...
    # ─── POLICE DASHBOARD & FILTERING (REQ-26, 27, 28) ───

    @app.route('/police_dashboard', methods=['GET', 'POST'])
    def police_dashboard():
        conn = get_db_connection()

        # Search & Filter Parameters
        search_crime = request.args.get('crime_type', '')
        ...
        high_priority = conn.execute("""
            ...
            FROM Complaints c JOIN Cases cs ON c.complaint_id = cs.complaint_id JOIN CrimeCategories cc ON c.category_id = cc.category_id
            WHERE cs.priority = 'High' ORDER BY c.created_at DESC LIMIT 4
        """).fetchall()

        categories = conn.execute("SELECT name FROM CrimeCategories").fetchall()
        conn.close()

        return render_template('police_dashboard.html',
                               pending=pending, in_progress=in_progress, resolved=resolved,
                               high_priority=high_priority, all_complaints=all_complaints,
                               categories=categories, search_crime=search_crime, search_status=search_status,
                               search_date=search_date)

    # ─── DETECTIVE DASHBOARD (REQ-14, REQ-11) ───
    ...
    # ─── OPERATOR & EMERGENCY DISPATCH (REQ-16, 17, 18) ───
    ...
    # ─── VOLUNTEER SYSTEM (REQ-19, 20) ───
    # Note: Modified to use Citizens table directly as per your schema constraints
    ...
    def volunteer_register():
        if request.method == 'POST':
            ...
            if not full_name or not cnic: flash("Name and CNIC required", "error"); return redirect(
                url_for('volunteer_register'))

            conn = get_db_connection()
            citizen = conn.execute("SELECT citizen_id FROM Citizens WHERE cnic = ?", (cnic,)).fetchone()

            if not citizen:
                cur = conn.execute("INSERT INTO Citizens (full_name, cnic) VALUES (?, ?)", (full_name, cnic))
                citizen_id = cur.lastrowid
            else:
                citizen_id = citizen['citizen_id']

            # Check if already a volunteer
            existing = conn.execute("SELECT volunteer_id FROM VolunteerDetails WHERE volunteer_id = ?",
                                    (citizen_id,)).fetchone()
            if existing:
                conn.close()
                flash("Already registered as a volunteer.", "error")
                return redirect(url_for('volunteer_register'))

            conn.execute("INSERT INTO VolunteerDetails (volunteer_id, skills) VALUES (?, ?)", (citizen_id, skills))
            conn.execute("INSERT INTO Notifications (message) VALUES (?)", (f'New volunteer registered: {full_name}.'))
            conn.commit()
            conn.close()
    ...
    @app.route('/assign_volunteer/<int:complaint_id>', methods=['POST'])
    def assign_volunteer(complaint_id):
        volunteer_id = request.form.get('volunteer_id')
        if not volunteer_id:
            flash("Select a volunteer", "error")
            return redirect(url_for('police_dashboard'))

        conn = get_db_connection()
        conn.execute("INSERT INTO VolunteerAssignments (volunteer_id, complaint_id) VALUES (?, ?)",
                     (volunteer_id, complaint_id))
        conn.execute("INSERT INTO Notifications (message) VALUES (?)",
                     (f'Volunteer assigned to Complaint #{complaint_id}.'))
        conn.commit()
        conn.close()
    ...
    def assign_case(complaint_id):
        ...
        conn = get_db_connection()
        case = conn.execute("SELECT case_id FROM Cases WHERE complaint_id = ?", (complaint_id,)).fetchone()
        if case:
            conn.execute(
    ...
    def resolve_case(complaint_id):
        ...
        conn.execute("UPDATE Cases SET last_updated = CURRENT_TIMESTAMP WHERE complaint_id = ?", (complaint_id,))
        conn.execute("INSERT INTO Logs (user_id, action) VALUES (?, ?)",
    ...
    def update_status(id):
        ...
        conn = get_db_connection()
        conn.execute("UPDATE Complaints SET status = ? WHERE complaint_id = ?", (status, id))
        conn.commit()
        conn.close()
        return redirect(url_for('police_dashboard'))

    # ─── STATISTICS BOARD (REQ-23, 24, 25) ───
...
def dispatch_unit():
    ...
    conn = get_db_connection()
    # Insert dispatch record
    conn.execute("INSERT INTO Dispatch (complaint_id, assigned_unit_id, status, eta) VALUES (?, ?, 'Dispatched', ?)",
                 (complaint_id, unit_id, eta))
    # Update complaint status
    conn.execute("UPDATE Complaints SET status = 'In Progress' WHERE complaint_id = ?", (complaint_id,))
    conn.execute("INSERT INTO Notifications (message) VALUES (?)",
...
def update_dispatch(dispatch_id):
    ...
    conn.execute("UPDATE Dispatch SET status = ? WHERE dispatch_id = ?", (new_status, dispatch_id))
    if new_status == 'Completed':
        dispatch = conn.execute("SELECT complaint_id FROM Dispatch WHERE dispatch_id = ?", (dispatch_id,)).fetchone()
        if dispatch:
            conn.execute("UPDATE Complaints SET status = 'Resolved' WHERE complaint_id = ?", (dispatch['complaint_id'],))
    conn.commit()
    conn.close()
    return redirect(url_for('admin_dashboard'))
...
def assign_case(complaint_id):
    ...
    case = conn.execute("SELECT case_id FROM Cases WHERE complaint_id = ?", (complaint_id,)).fetchone()
    if case:
        conn.execute("UPDATE Cases SET assigned_police_id = ?, last_updated = CURRENT_TIMESTAMP WHERE complaint_id = ?", (session['user_id'], complaint_id))
        conn.execute("UPDATE Complaints SET status = 'In Progress' WHERE complaint_id = ?", (complaint_id,))
        conn.execute("INSERT INTO Logs (user_id, action) VALUES (?, ?)", (session['user_id'], f"Assigned case #{complaint_id}"))
        conn.commit()
        flash(f'Case #{complaint_id} assigned to you.', 'success')
    conn.close()
    return redirect('/police_dashboard')
...
def resolve_case(complaint_id):
    ...
    conn.execute("UPDATE Complaints SET status = 'Resolved' WHERE complaint_id = ?", (complaint_id,))
    conn.execute("UPDATE Cases SET last_updated = CURRENT_TIMESTAMP WHERE complaint_id = ?", (complaint_id,))
    conn.execute("INSERT INTO Logs (user_id, action) VALUES (?, ?)", (session['user_id'], f"Resolved case #{complaint_id}"))
    conn.commit()
    conn.close()
    flash(f'Case #{complaint_id} marked as Resolved.', 'success')
    return redirect('/police_dashboard')
